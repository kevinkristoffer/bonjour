...
class Bonjour_Model_PrivilegeGateway extends Bonjour_Core_Model_GateWay{
	 ...
	 * 版块权限管理
	 * 版块权限只与用户组和用户有关
	 * 在权限主表中的accessName='Forum',operation='NAVIGATE',condition{forumID in (?,?,?,?)}
	 * 版块权限管理有两种模式（操作权限管理也是类似）
	 * 1、版块模式：点击某个版块能看到这个版块下的全部访问主体[user,role,team]，只能移除权限操作
	 * 2、主体模式：能根据主体标示[userID,roleID]分类[group]生成主体列表，
	 ...
	 * 管理模式与版块权限管理类似
	 * 2、局部操作权限
	 * 对象创建者（如项目、需求等）拥有局部操作权限
	 * 当然还有很多符合逻辑的特殊局部权限是固定在程序中（如所有团队成员能相互访问团队项目，负责人能拥有该项目全部权限等）
	 * 对象创建者具有将全局操作权限拥有者拉入黑名单的权利
	 * 
	 */
