	...
	public function addProjectAction() {
		...
		if ($this->_request->isPost ()) {
			...
			try {
				...
				if (! isset ( $projectName ) || strlen ( $projectName ) < 3 || strlen ( $projectName ) > 100)
					throw new Exception ();
				...
				if (isset ( $estimateDuration ) && $estimateDuration <= 0)
					throw new Exception ();
				$responsible = $this->_request->getParam ( 'f05' );
				if (! isset ( $responsible ) || ! preg_match ( '/\|/', $responsible ))
					throw new Exception ();
				...
				try {
					...
					$project = array (
							...
							'rootNode' => $rootNode,
							'parentNode' => $parentNode,
							'nodeCodeRoute' => $nodeCodeRoute,
							'nodeNameRoute' => $nodeNameRoute,
